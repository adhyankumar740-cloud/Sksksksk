# leaderboard_manager.py (MODIFIED to store quiz counter in DB)
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                ...
                # --- 💡 NEW: Add quiz_message_count column ---
                try:
                    cur.execute("SELECT quiz_message_count FROM chats LIMIT 1;")
                except psycopg2.ProgrammingError:
                    conn.rollback()
                    logger.warning("Adding 'quiz_message_count' column to 'chats' table.")
                    cur.execute("ALTER TABLE chats ADD COLUMN quiz_message_count INT DEFAULT 0 NOT NULL;")
                    conn.commit()
                # --- End NEW ---
