# leaderboard_manager.py (FINAL & ROBUST VERSION WITH CHARACTER CLEANING)
...
# Zyadatar problems NotoSans file daalne se theek ho jaati hain.
FONT_MAIN = "Roboto-Bold.ttf"
FONT_NAMES = "NotoSans-Regular.ttf" # Most robust font for all languages/symbols
FONT_FALLBACK = "arial.ttf" # Common Windows/Linux fallback font 
...
def clean_username_for_image(name: str) -> str:
    """
    Removes unsupported/fancy characters that cause 'boxes' [] in PIL.
    """
    # Allowed characters: English letters, numbers, basic punctuation, spaces
    # This strips out most designer symbols (like small caps, fancy borders)
    # The output is simple ASCII/Latin characters.
    cleaned_name = ''.join(c for c in name if c.isalnum() or c in ' -_().@#')
    
    # Optional: Truncate if the name is still excessively long after cleaning
    if len(cleaned_name) > 30:
        return cleaned_name[:27] + '...'
...
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str, total_count: int):
    ...
    def get_font(name, size):
        try:
            ...
            # Fallback 1: Try Arial if NotoSans is missing
            try:
                return ImageFont.truetype(FONT_FALLBACK, size)
            except:
                # Fallback 2: Default font (will likely show boxes, but prevents crash)
                return ImageFont.load_default()

    # Load Fonts
    font_title = get_font(FONT_MAIN, 50)  
    font_sub = get_font(FONT_MAIN, 30)    
    font_text = get_font(FONT_NAMES, 36) # Using the most robust font for names
    ...
    for i, (name, count) in enumerate(leaderboard_data):
        ...
        # 3. Username (CLEANED NAME used here)
        cleaned_name = clean_username_for_image(name) # 💡 Clean the name before drawing
        d.text((160, y_pos), cleaned_name, font=font_text, fill=row_color)

        d.line([(60, y_pos + 60), (IMG_WIDTH - 60, y_pos + 60)], fill=COLOR_BG, width=1)
...
def format_leaderboard_text(title: str, chat_name: str, data: list, total_count: int):
    ...
    for i, (name, count) in enumerate(data):
        ...
        # We don't clean name here, Telegram can handle Unicode fine in text
