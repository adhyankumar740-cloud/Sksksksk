# leaderboard_manager.py (Final Version)
...
# 💡 FIX 1: Placeholder URL को एक मान्य इमेज से बदला गया ताकि 'Wrong type...' एरर न आए।
# अगर आप अपना URL सेट नहीं करते हैं, तो यह डिफ़ॉल्ट रूप से इस URL का उपयोग करेगा।
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                # ... (Table setup logic remains the same) ...
                cur.execute("""
            ...
            if conn: # 💡 Safety check: ensure connection closure
                conn.close()
...
async def get_leaderboard_data(chat_id: int, scope: str):
    ...
    # ... (Time and chat filter logic remains the same) ...
    time_filter = ""
    chat_filter = ""
    title = ""
...
# --- Leaderboard Command (Photo fallback improved) ---
...
async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Attempt 1: Send Photo with caption
    try:
        ...
        # If photo fails (e.g., Wrong type of the web page content), send text fallback
        logger.warning(f"Failed to send leaderboard photo, sending text fallback: {e}")
        try:
            text_message = await update.message.reply_text(initial_caption, parse_mode=constants.ParseMode.MARKDOWN)
            photo_message_id = text_message.message_id
    ...
    if photo_message_id:
        try:
            # If a photo message ID was obtained (either from photo or text fallback), edit it.
            # We attempt edit_message_caption first if we sent a photo, but we must handle the case where we sent text.
            if LEADERBOARD_PHOTO_URL_KEY and "placeholder" not in LEADERBOARD_PHOTO_URL_KEY:
                 # Try editing caption (for photo)
                 ...
                 # Use edit_message_text (for text fallback or invalid photo)
             ...
             logger.error(f"Failed to edit message ID {photo_message_id} after fallback: {e}. Sending new message.")
             # Final fallback: Send a new message if editing failed
             await update.message.reply_text(leaderboard_text, reply_markup=reply_markup, parse_mode=constants.ParseMode.MARKDOWN)
    else:
        # If no initial message could be sent at all
...
# --- Leaderboard Callback (Caption Error Fixed) ---
...
async def leaderboard_callback(update: Update, context: CallbackContext):
    ...
    try:
        ...
        # 💡 FIX 2: Use edit_message_text for safer handling of invalid data.
        try:
            await query.edit_message_text(text="Invalid button data.")
        except:
            pass # Ignore if we can't edit an invalid message.
        return
    ...
    try:
        # 💡 FIX 3: Use edit_message_text to avoid "There is no caption..." error
        ...
        logger.error(f"Error during final edit_message_text: {e}")
...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ... (rest of profile command remains the same) ...
    user = update.effective_user
    user_id = user.id
    username = f"@{user.username}" if user.username else user.first_name
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ... (rest of broadcast command remains the same) ...
    if not chat_ids:
