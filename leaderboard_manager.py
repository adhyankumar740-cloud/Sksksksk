# leaderboard_manager.py (FINAL VERSION 15: Typo Fix, Profile Photo Feature, Lighter LB Divider)
...
# --- Database Setup (MODIFIED: Fixed typo 'check_and_column' and restored) ---
def setup_database():
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                ...
                # FIX: Typo corrected (as per previous turn's fix)
...
# --- 🖼️ Leaderboard Image Generator (MODIFIED: Lighter Divider Line & Compactness) ---
...
# --- Leaderboard Core Logic (MODIFIED: Added user_stats retrieval) ---
async def get_leaderboard_data(chat_id: int, scope: str, current_user_id: int = None):
    conn = get_db_connection()
    if not conn:
        # Added None for user_stats
...
# --- Format Leaderboard Text (MODIFIED: Added user stats, removed extra lines) ---
...
# --- Helper function for ticked buttons (MODIFIED: Added Global button) ---
...
# --- Ranking Command (MODIFIED: Added user_stats and global scope option) ---
...
# --- Leaderboard Callback (MODIFIED: Added user_stats) ---
...
# --- Profile Command (MODIFIED: Added Profile Photo Fetching and Sending) ---
