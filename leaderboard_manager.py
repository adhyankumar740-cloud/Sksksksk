# leaderboard_manager.py (MODIFIED for Real Names and Larger Image)
...
# --- Message Count Update (MODIFIED to save real name) ---
async def update_message_count_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- MODIFICATION START ---
    # Save the user's First Name or Full Name, not username
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                VALUES (%s, %s, %s);
            """, (chat_id, user_id, display_name)) # <-- MODIFIED
        conn.commit()
    except Exception as e:
...
# --- Leaderboard Image Generator (MODIFIED for Size) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str):
    try:
        # --- MODIFIED FONT SIZES ---
        font_title = ImageFont.truetype(FONT_FILE_BOLD, 48) # was 40
        font_chat = ImageFont.truetype(FONT_FILE_REGULAR, 30) # was 24
        font_rank = ImageFont.truetype(FONT_FILE_BOLD, 34) # was 28
        font_user = ImageFont.truetype(FONT_FILE_REGULAR, 34) # was 28
    except IOError as e:
    ...
    # --- MODIFIED DIMENSIONS ---
    item_height = 60 # was 50
    header_height = 180 # was 150
    footer_height = 60 # was 50
    min_content_height = 300 # was 200
    # ---
    ...
    # --- Draw Header (MODIFIED coordinates) ---
    title_bbox = d.textbbox((0, 0), title, font=font_title)
    title_w = title_bbox[2] - title_bbox[0]
    d.text(((IMG_WIDTH - title_w) / 2, 50), title, font=font_title, fill=TITLE_COLOR) # y was 40
    ...
    d.text(((IMG_WIDTH - chat_w) / 2, 120), chat_name_text, font=font_chat, fill=TEXT_COLOR) # y was 100

    # --- Draw Leaderboard Entries (MODIFIED coordinates) ---
    if not leaderboard_data:
        ...
        y_pos = header_height + 10 # Start below the header (added 10 padding)
        for i, (display_name, count) in enumerate(leaderboard_data):
            rank = f"{i+1}."
            # Increased character limit slightly
            username_display = f"{display_name[:25]}" + ('...' if len(display_name) > 25 else '')
            count_str = f"{count} msgs"

            # --- MODIFIED X-COORDINATES ---
            # Rank
            d.text((60, y_pos), rank, font=font_rank, fill=RANK_COLOR) # x was 50
            # Username
            d.text((150, y_pos), username_display, font=font_user, fill=TEXT_COLOR) # x was 120

            # Count
            count_bbox = d.textbbox((0, 0), count_str, font=font_user)
            count_w = count_bbox[2] - count_bbox[0]

            d.text( (IMG_WIDTH - count_w - 60, y_pos), count_str, font=font_user, fill=TEXT_COLOR) # x was -50
...
# --- Leaderboard Core Logic (MODIFIED to GROUP BY USER_ID) ---
async def get_leaderboard_data(chat_id: int, scope: str):
    ...
    # --- MODIFIED SQL QUERY ---
    # This query now groups by the unique 'user_id' to consolidate scores
    # and uses a subquery to fetch the *latest* name ('username' column) for that user.
    query = f"""
...
# --- Leaderboard Command (Unchanged, already correct) ---
async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id

    try:
        sent_message = await update.message.reply_text("🏆 Generating leaderboard, please wait...")
    except Exception as e:
...
# --- Leaderboard Callback (Unchanged, already correct) ---
...
# --- Profile Command (MODIFIED to use new Name logic) ---
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Get user's current name and escape it
