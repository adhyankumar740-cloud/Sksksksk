# leaderboard_manager.py (FINAL VERSION 17: Added IST timezone logic for daily/weekly leaderboards)
...
import re 
import pytz # ADDED pytz for IST timezone handling

logger = logging.getLogger(__name__)

# --- ⚙️ Image & Font Configuration (Unchanged from V15) ---
...
# --- Database Setup (Unchanged from V15) ---
def setup_database():
...
# --- 🖼️ Leaderboard Image Generator (Unchanged from V15) ---
...
# --- Leaderboard Core Logic (MODIFIED for IST) ---
async def get_leaderboard_data(chat_id: int, scope: str, current_user_id: int = None):
    ...
    # The WHERE clause here must use the same number of %s placeholders as there are items in the 'params' list.
    # Since the structure is fixed, we can embed the WHERE clause and rely on psycopg2 for substitution.
    query = f"""
...
# --- Format Leaderboard Text (Unchanged from V15) ---
...
# --- Helper function for ticked buttons (Unchanged from V15) ---
...
# --- Ranking Command (Unchanged from V15) ---
...
# --- Leaderboard Callback (Unchanged from V15) ---
...
# --- Profile Command (Unchanged from V15) ---
...
# --- Utility Functions (MODIFIED: Added get_chat_stats) ---
def get_chat_stats():
    """
