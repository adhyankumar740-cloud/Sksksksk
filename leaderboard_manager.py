# leaderboard_manager.py (FINAL & ROBUST VERSION WITH ID FALLBACK)
...
import re # For cleaning/checking usernames
...
# --- Note: Ensure '25552 (1).jpg', 'Roboto-Bold.ttf', and 'Roboto-Regular.ttf' are in the bot directory ---
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                ...
                # Dynamic column checks (for pre-existing DBs)
...
# --- Message Count Update (Unchanged logic - still stores display name) ---
async def update_message_count_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Stores display name which leads to the box issue, hence the fallback is needed later.
...
# --- Leaderboard Image Generator (MODIFIED: Fallback Logic & No "User ID:" prefix) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str):
    # Helper to safely load fonts
    def get_font(name, size):
        try:
            return ImageFont.truetype(name, size)
        except IOError as e:
            logger.warning(f"Font file '{name}' not found. Falling back to default.")
            return ImageFont.load_default()

    font_title = get_font(FONT_FILE_BOLD, 48)
    font_rank = get_font(FONT_FILE_BOLD, 34)
    font_user = get_font(FONT_FILE_REGULAR, 34)
    ...
    try:
        ...
        if bg_img.height < total_height:
             img = Image.new('RGB', (IMG_WIDTH, total_height), color=(15, 23, 42)) # Fallback BG color
        ...
        img = Image.new('RGB', (IMG_WIDTH, total_height), color=(15, 23, 42)) 
    ...
    # --- Draw Leaderboard Entries (MODIFIED: Fallback Logic) ---
    if not leaderboard_data:
        ...
        # Data is now (display_name, count, user_id)
        for i, (display_name, count, user_id) in enumerate(leaderboard_data):
            rank = f"#{i+1}" 
            
            # 💡 NEW FALLBACK LOGIC: Check for empty name or non-ASCII characters (to catch unrenderable Unicode/boxes)
            # The simplest check is if the name contains mostly non-Latin characters which the font fails to render.
            # We use a robust regex to check if the name is composed primarily of basic alphanumeric/space characters.
            
            clean_display_name = display_name.strip()
            # If the name is empty OR if it contains less than 50% basic printable ASCII characters (a heuristic for box/garbage names)
            is_unrenderable = not clean_display_name or len(re.sub(r'[\w\s]', '', clean_display_name)) > (len(clean_display_name) / 2)
            
            if is_unrenderable:
                # Use raw user_id number as display name
                username_display = str(user_id) 
            else:
                # Use stored display name (truncated)
...
# --- Leaderboard Core Logic (MODIFIED: Now includes user_id in SELECT) ---
async def get_leaderboard_data(chat_id: int, scope: str):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(query)
            # Results are now (username, count, user_id)
...
# --- Format Leaderboard Text (MODIFIED: Fallback Logic) ---
def format_leaderboard_text(title: str, chat_name: str, data: list):
    ...
    for rank, (display_name, count, user_id) in enumerate(data, 1):
        ...
        if is_unrenderable:
            # Use raw user_id number for fallback in text too
            username_display = str(user_id) 
        else:
...
# --- Ranking Command (Unchanged logic, now handles new data format) ---
...
# --- Leaderboard Callback (Unchanged logic, now handles new data format) ---
