# leaderboard_manager.py (FINAL VERSION 10: Dynamic Image Height Fix, Smaller Image, No # in Rank, New 3+1 Button Layout)
...
IMG_WIDTH = 900  # Reduced from 1000
HEADER_HEIGHT = 160  
ROW_HEIGHT = 45      # Reduced from 50
FOOTER_HEIGHT = 40   
...
# --- 🖼️ Leaderboard Image Generator (MODIFIED: Dynamic Height Fix, Smaller Size, No #) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str, total_count: int):
    ...
    try:
        ...
        # Paste the resized background image from the top
    ...
    # --- Draw Header (No trophy emoji in image) ---
    ...
    sub_text = f"📊 Total Messages: {total_count}" # Keep total count for context
    ...
    for i, (display_name, count, user_id) in enumerate(leaderboard_data):
        ...
        # Vertical position for text (Adjusted for new smaller ROW_HEIGHT)
        y_text_pos = y_pos + 5 

        # 1. Rank Number (REMOVED #)
...
async def get_leaderboard_data(chat_id: int, scope: str):
    ...
    if scope == 'global':
        ...
        title = "This Chat All-Time Legends" # Title updated to match button name
        chat_filter = f"chat_id = {chat_id}"
    else:
...
# --- Format Leaderboard Text (MODIFIED: Removed all extra emojis/subtitles from caption) ---
...
# --- Helper function for ticked buttons (MODIFIED: New 3+1 Layout, Clean Checkmarks) ---
def create_leaderboard_keyboard(scope: str, chat_id: int):
    daily_text = "Today"
    weekly_text = "Weekly"
    this_chat_text = "This Chat" # Local All-Time
    global_text = "Global Ranking"
    
    # 1. Handle checkmarks for Row 1 buttons (Local scopes)
    if scope == 'daily':
    ...
    # 2. Handle checkmark for Row 2 button (Global scope)
    elif scope == 'global':
        global_text = f"✅ {global_text}"
        
    keyboard = [
        # Row 1: Today, Weekly, This Chat (Local All-Time)
