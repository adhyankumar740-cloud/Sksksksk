...
# Note: Use an actual Telegram File ID or URL here
...
# --- Leaderboard Core Logic (FIXED SQL WHERE) ---
...
async def leaderboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Note: We send an initial message with the photo, and then edit its caption
        ...
        # If photo fails, send a plain message to get a message ID to edit later
        text_message = await update.message.reply_text("🏆 Loading Leaderboard... 📊")
        photo_message_id = text_message.message_id
    except Exception as e:
        logger.error(f"Error sending photo or text message: {e}")
        photo_message_id = None
    ...
    if photo_message_id:
        try:
            # We assume the initial message (photo or text) has a caption/text to edit
            # Use edit_message_caption if we know a photo was sent, otherwise we risk an error.
            # However, for simplicity and safety, we rely on the callback to handle the subsequent edits.
            
            # Since we want to use the same logic in callback (edit_message_text), 
            # we will attempt to edit the caption, and if it fails, we fall back to a new reply.
            ...
            await context.bot.edit_message_caption(
             ...
             logger.error(f"Failed to edit photo caption, sending new message: {e}")
...
# --- Leaderboard Callback (FIXED CAPTION ERROR) ---
...
async def leaderboard_callback(update: Update, context: CallbackContext):
    ...
    try:
        # Check if the message is too old or if the structure is broken
        if not query.data or len(query.data.split('_')) < 2 or len(query.data.split('_')[1].split(':')) < 2:
            raise ValueError("Invalid callback data format.")
            
        scope, chat_id_str = query.data.split('_')[1].split(':')
        chat_id = int(chat_id_str)
    except Exception:
        # 💡 FIX: Use edit_message_text for safer handling of messages without captions
        try:
            await query.edit_message_text(text="Invalid button data.")
        except Exception as e:
            logger.error(f"Failed to edit message text on invalid data: {e}")
        return
    ...
    try:
        # 💡 FIX: Use edit_message_text here as well to handle both photo and plain text message edits safely
