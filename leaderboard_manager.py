# leaderboard_manager.py (FINAL VERSION 13: Profile Command Fix, Show Current User Stats in LB)
...
# --- ⚙️ Image & Font Configuration (Unchanged from V12) ---
...
# --- 🖼️ Leaderboard Image Generator (Unchanged from V12: Strict BG Fix, Smaller Font) ---
...
# --- Leaderboard Core Logic (MODIFIED: Added current_user_id to signature and fetching user stats) ---
async def get_leaderboard_data(chat_id: int, scope: str, current_user_id: int = None):
    ...
    if current_user_id:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(user_stats_query)
                stats_result = cur.fetchone()
                if stats_result:
                    # (rank, count) - rank is float from RANK(), count is int/long
...
# --- Format Leaderboard Text (MODIFIED: Added user stats display) ---
def format_leaderboard_text(title: str, chat_name: str, data: list, total_count: int, user_stats: tuple, current_user_name: str):
    ...
    if user_stats:
        ...
        leaderboard_text += f"👤 *{escaped_user_name}'s Stats ({title})*\n"
        leaderboard_text += "━━━━━━━━━━━━━━━━━━\n"
        leaderboard_text += f"*{title.split(' ')[0]}* Rank: `{rank}`\n" # Displays Today/Weekly/All-Time/Global Rank
...
# --- Ranking Command (MODIFIED: Pass user ID/name and handle new return value) ---
...
# --- Leaderboard Callback (MODIFIED: Pass user ID/name and handle new return value) ---
async def leaderboard_callback(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if "Message is not modified" not in str(e):
            # This is where the old query error appeared. With new logic, this should be minimal.
...
# --- Profile Command (MODIFIED: Escaping reserved characters for Markdown V2) ---
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX 1: Escape parentheses and asterisks in the profile text template
    ...
    try:
        with conn.cursor() as cur:
            ...
            # FIX 2: Escape parentheses in the stats line
