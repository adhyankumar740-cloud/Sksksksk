# leaderboard_manager.py (FINAL VERSION 3: Compact Image + Verified Caption Fallback)
...
# Image Dimensions (MODIFIED FOR COMPACTNESS)
IMG_WIDTH = 1200
HEADER_HEIGHT = 180  # Reduced from 220
ROW_HEIGHT = 60      # Reduced from 70
FOOTER_HEIGHT = 50   # Reduced from 60
...
# --- 🖼️ Leaderboard Image Generator (MODIFIED: Compact Header/Row Heights) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str, total_count: int):
    ...
    # --- Draw Header (Position Adjusted) ---
    title_bbox = d.textbbox((0, 0), title, font=font_title)
    title_w = title_bbox[2] - title_bbox[0]
    d.text(((IMG_WIDTH - title_w) / 2, 40), title, font=font_title, fill=COLOR_TITLE) # Adjusted

    sub_text = f"📊 Total Group Messages: {total_count}"
    sub_bbox = d.textbbox((0, 0), sub_text, font=font_sub)
    sub_w = sub_bbox[2] - sub_bbox[0]
    d.text(((IMG_WIDTH - sub_w) / 2, 100), sub_text, font=font_sub, fill=COLOR_SUBTITLE) # Adjusted
    ...
    for i, (display_name, count, user_id) in enumerate(leaderboard_data):
        ...
        # Vertical position for text (centered in ROW_HEIGHT=60, font size=36)
        y_text_pos = y_pos + 12 
        ...
        d.line([(60, y_pos + 50), (IMG_WIDTH - 60, y_pos + 50)], fill=COLOR_BG, width=1) # Adjusted line position
        
        y_pos += ROW_HEIGHT # Increments by 60
...
# --- Format Leaderboard Text (Verified Loosened Fallback Logic) ---
def format_leaderboard_text(title: str, chat_name: str, data: list, total_count: int):
    ...
    for rank, (display_name, count, user_id) in enumerate(data, 1):
        ...
        # or has NO letters/numbers (which prevents unreadable boxes '□□□□' in the text caption).
