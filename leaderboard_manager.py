# leaderboard_manager.py (FINAL VERSION: Image Fixes, Robust ID Fallback, Dynamic Sizing)
...
# --- Database Setup (Unchanged, kept all columns check) ---
def setup_database():
...
# --- 🖼️ Leaderboard Image Generator (Re-integrated Robust Logic + Fallback) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str, total_count: int):
    ...
    for i, (display_name, count, user_id) in enumerate(leaderboard_data):
        ...
        # Display Name Logic: Fallback to User ID if name is problematic
...
# --- Leaderboard Core Logic (Kept the version that fetches user_id) ---
...
# --- Format Leaderboard Text (MODIFIED: Strict Fallback Logic) ---
def format_leaderboard_text(title: str, chat_name: str, data: list, total_count: int):
    ...
    for rank, (display_name, count, user_id) in enumerate(data, 1):
        ...
        # Stricter check: if the name is empty or lacks letters/numbers/spaces, use ID
        if not display_name or not re.search(r'[a-zA-Z0-9\s]', display_name):
            username_display = str(user_id) 
        else:
...
# --- Ranking Command (Updated to use total_count) ---
...
# --- Leaderboard Callback (Updated to use total_count) ---
