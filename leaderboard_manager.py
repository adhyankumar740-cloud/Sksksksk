# leaderboard_manager.py (FINAL VERSION 12: Profile Command Fix, Strict BG Fix, Smaller Fonts)
...
# --- ⚙️ Image & Font Configuration (Modified Image Size & Font) ---
FONT_MAIN = "Roboto-Bold.ttf"
FONT_NAMES = "NotoSans-Regular.ttf" 
FONT_FALLBACK = "arial.ttf" 
BACKGROUND_IMAGE_PATH = "25552 (1).jpg" 

# Image Dimensions (FURTHER REDUCED FOR SMALLER SIZE)
IMG_WIDTH = 900  
HEADER_HEIGHT = 160  
ROW_HEIGHT = 40      # Reduced from 45
FOOTER_HEIGHT = 40   
...
# --- 🖼️ Leaderboard Image Generator (MODIFIED: Smaller Font, Strict BG Fix) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str, total_count: int):
    ...
    # Load Fonts (REDUCED SIZE)
    font_title = get_font(FONT_MAIN, 45)  # Reduced from 50
    font_sub = get_font(FONT_MAIN, 28)    # Reduced from 30
    font_text = get_font(FONT_NAMES, 24) # Reduced from 30
    font_rank = get_font(FONT_MAIN, 26) # Reduced from 32
    ...
    # --- Draw Header (Position Adjusted for smaller title font) ---
    title_bbox = d.textbbox((0, 0), title, font=font_title)
    title_w = title_bbox[2] - title_bbox[0]
    d.text(((IMG_WIDTH - title_w) / 2, 35), title, font=font_title, fill=COLOR_TITLE) # Y pos adjusted

    sub_text = f"📊 Total Messages: {total_count}" 
    sub_bbox = d.textbbox((0, 0), sub_text, font=font_sub)
    sub_w = sub_bbox[2] - sub_bbox[0]
    d.text(((IMG_WIDTH - sub_w) / 2, 95), sub_text, font=font_sub, fill=COLOR_SUBTITLE) # Y pos adjusted
    ...
    for i, (display_name, count, user_id) in enumerate(leaderboard_data):
        ...
        # Vertical position for text (Adjusted for new smaller ROW_HEIGHT=40 and smaller font)
        y_text_pos = y_pos + 6 
...
# --- Leaderboard Core Logic (MODIFIED: Title change for alltime) ---
async def get_leaderboard_data(chat_id: int, scope: str):
    ...
    if scope == 'global':
        ...
        title = "All-Time Legends (Local Chat)" # Title changed to match 'All-Time' button
        chat_filter = f"chat_id = {chat_id}"
    else:
...
# --- Format Leaderboard Text (Unchanged clean version) ---
...
# --- Helper function for ticked buttons (Unchanged from V11) ---
def create_leaderboard_keyboard(scope: str, chat_id: int):
    daily_text = "Today"
    weekly_text = "Weekly"
    alltime_local_text = "All-Time" # Row 1 button (Local All-Time)
    
    # Row 2 buttons
    this_chat_text = "This Chat" # Row 2 Left button (Local All-Time)
    global_text = "Global Ranking" # Row 2 Right button
    ...
    # Both 'All-Time' and 'This Chat' map to 'alltime' scope.
    if scope == 'alltime':
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(this_chat_text, callback_data=f"lb_alltime:{chat_id}"), # Maps to 'alltime' scope
         InlineKeyboardButton(global_text, callback_data=f"lb_global:{chat_id}")] # Maps to 'global' scope
    ]
    return InlineKeyboardMarkup(keyboard)

# --- Ranking Command (Unchanged logic, uses modified functions) ---
...
# --- Profile Command (MODIFIED: Query Made Robust with LEFT JOIN and COALESCE) ---
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Query modified to use LEFT JOIN and COALESCE for robustness
            cur.execute("""
            ...
            if not group_stats:
                ...
                for chat_id, chat_name, count in group_stats:
                    # Escape chat name (which might be 'Unknown Chat')
        ...
        # The improved query should fix the issue, but if another error occurs, this fallback remains.
