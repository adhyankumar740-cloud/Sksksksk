# leaderboard_manager.py (FINAL VERSION 20: FIXED all known unsafe fetchone()[0] errors)
...
async def get_leaderboard_data(chat_id: int, scope: str, current_user_id: int = None):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # V19 FIX: Safely fetch total_count to prevent list index out of range/NoneType error
            ...
            if scope != 'global':
                ...
                # FIX: Safely fetch chat_name to prevent list index out of range/NoneType error
                if chat_name_result:
