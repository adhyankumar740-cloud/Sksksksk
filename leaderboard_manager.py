# leaderboard_manager.py (FINAL VERSION 9: Dynamic Image Height, Modified Button Layout, DM Restriction)
...
# --- ⚙️ Image & Font Configuration (Modified Image Size) ---
FONT_MAIN = "Roboto-Bold.ttf"
FONT_NAMES = "NotoSans-Regular.ttf" 
FONT_FALLBACK = "arial.ttf" 
BACKGROUND_IMAGE_PATH = "25552 (1).jpg" 

# Image Dimensions (REDUCED FOR SMALLER SIZE)
IMG_WIDTH = 1000  # Width reduced
HEADER_HEIGHT = 160  
ROW_HEIGHT = 50      # Row height reduced
FOOTER_HEIGHT = 40   
...
# --- 🖼️ Leaderboard Image Generator (MODIFIED: Dynamic Height and BG Resize Fix) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str, total_count: int):
    ...
    # --- Load Background Image (Modified Logic to stretch/crop to total_height) ---
    try:
        bg_img = Image.open(BACKGROUND_IMAGE_PATH).convert("RGB")
        
        # Resize BG to match IMG_WIDTH, maintaining aspect ratio.
        bg_width, bg_height = bg_img.size
        width_ratio = IMG_WIDTH / bg_width
        
        new_bg_width = IMG_WIDTH
        new_bg_height = int(bg_height * width_ratio)
        
        bg_img = bg_img.resize((new_bg_width, new_bg_height), Image.LANCZOS)
        
        img = Image.new('RGB', (IMG_WIDTH, total_height), color=COLOR_BG)
        
        # Paste the resized background image. If new_bg_height < total_height, the remaining area will be COLOR_BG.
    ...
    d.line([(60, HEADER_HEIGHT - 20), (IMG_WIDTH - 60, HEADER_HEIGHT - 20)], fill=COLOR_DIVIDER, width=2)
...
# --- Leaderboard Core Logic (MODIFIED: Handles 'global' scope) ---
...
def format_leaderboard_text(title: str, chat_name: str, data: list, total_count: int):
    ...
    # Includes chat_name in subtitle for clarity, especially in Global mode
    if chat_name and chat_name != "All Registered Chats":
        subtitle_text = f"🌐 {escape_markdown(chat_name, version=2)}"
    else:
        subtitle_text = f"🌎 {escape_markdown(chat_name, version=2)}"
        

    leaderboard_text = f"*{safe_title}* 🏆\n"
    leaderboard_text += f"*{subtitle_text}* \n"
...
# --- Helper function for ticked buttons (MODIFIED: New 3+1 Layout and Toggle Logic) ---
def create_leaderboard_keyboard(scope: str, chat_id: int):
    daily_text = "Today"
    weekly_text = "Weekly"
    alltime_text = "All-Time" # Local All-Time
    
    # Logic for the Toggle Button (Local All-Time <-> Global All-Time)
    if scope == 'global':
        # Currently showing Global. Button switches to 'alltime' (Local).
        toggle_text = "✅ Global Top (Tap for Local)"
        toggle_scope = "alltime"
    elif scope == 'alltime':
        # If 'alltime' (local) is explicitly selected via the toggle or initial button, show the Local checkmark on the toggle button.
        toggle_text = "✅ Local All-Time (Tap for Global)"
        toggle_scope = "global"
    else:
        # Default view for Today/Weekly
        toggle_text = "Global Top / Local All-Time" 
        toggle_scope = "global"
    
    # Handle checkmarks for Row 1 buttons
    if scope == 'daily':
        daily_text = f"✅ {daily_text}"
    elif scope == 'weekly':
        weekly_text = f"✅ {weekly_text}"
    # alltime_text does NOT get a checkmark if scope is 'alltime', as the checkmark is on the toggle button.

    # If 'alltime' is selected directly from Row 1, keep the checkmark there (This is a simpler UX compromise)
    if scope == 'alltime' and 'Local All-Time' not in toggle_text:
        alltime_text = f"✅ {alltime_text}"
        toggle_text = "Global Top / Local All-Time" # Reset toggle text if not active

    toggle_data = f"lb_{toggle_scope}:{chat_id}"
    ...
    keyboard = [
        ...
        # Row 2: Global/Local Toggle Button
        [InlineKeyboardButton(toggle_text, callback_data=toggle_data)]
    ]
    return InlineKeyboardMarkup(keyboard)

# --- Ranking Command (MODIFIED: Added DM restriction) ---
...
# --- Leaderboard Callback (Unchanged logic, uses modified functions) ---
