# leaderboard_manager.py (FINAL VERSION 8: Global/Local All-Time Toggle, Reduced image size, removed row lines)
...
# --- ⚙️ Image & Font Configuration (MODIFIED: Image Size Reduced) ---
...
IMG_WIDTH = 1000  # Changed from 1200
HEADER_HEIGHT = 160  
ROW_HEIGHT = 50      # Changed from 55
FOOTER_HEIGHT = 40   
...
# --- 🖼️ Leaderboard Image Generator (Modified: Text position adjusted for smaller ROW_HEIGHT) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str, total_count: int):
    ...
    for i, (display_name, count, user_id) in enumerate(leaderboard_data):
        ...
        # Display Name Logic (IN IMAGE): Keeps User ID fallback to prevent boxes in the image.
        ...
        # Vertical position for text (Adjusted from 10 to 7 due to smaller ROW_HEIGHT)
        y_text_pos = y_pos + 7 
        ...
        # Inter-row divider line removed as requested in the first step.
...
async def get_leaderboard_data(chat_id: int, scope: str):
    ...
    if scope == 'global':
        ...
        title = "All-Time Legends (Local Chat)" # Clarified title for local
        chat_filter = f"chat_id = {chat_id}"
    else:
...
def format_leaderboard_text(title: str, chat_name: str, data: list, total_count: int):
    ...
    for rank, (display_name, count, user_id) in enumerate(data, 1):
        ...
        # *** FINAL FIX: CAPTION NAME ***
        # Removed the User ID fallback. If the name is empty/null, it will show a blank name, 
        # thus removing the unwanted number.
        ...
        # If the name is totally empty after escaping, use a safe space.
...
# --- Helper function for ticked buttons (MODIFIED: Global/Local All-Time Toggle) ---
def create_leaderboard_keyboard(scope: str, chat_id: int):
    ...
    if scope == 'global':
        ...
        # Currently showing Local All-Time. Button switches to 'global'.
        toggle_text = "✅ Local All-Time (Tap for Global)"
        toggle_scope = "global"
    else:
        # Currently showing Today/Weekly. Button switches to 'global'.
        toggle_text = "Global Top / Local All-Time" 
        toggle_scope = "global"
    
    # Handle checkmarks for daily/weekly
    if scope == 'daily':
...
async def ranking_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Keyboard now has only 3 buttons
