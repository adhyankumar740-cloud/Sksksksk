...
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str):
    ...
    header_height = 150
    footer_height = 50
    total_height = header_height + (len(leaderboard_data) * item_height) + footer_height

    img = Image.new('RGB', (IMG_WIDTH, total_height), color=BG_COLOR)
    d = ImageDraw.Draw(img)
    ...
    if not leaderboard_data:
        ...
        d.text(((IMG_WIDTH - no_data_w) / 2, 180), no_data_text, font=font_user, fill=TEXT_COLOR)
    else:
        y_pos = header_height
