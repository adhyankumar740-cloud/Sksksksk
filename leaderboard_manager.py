# leaderboard_manager.py (MODIFIED to store ALL persistence in DB)
...
# --- Database Setup (💡 HEAVILY MODIFIED) ---
def setup_database():
    ...
    if conn:
        try:
            with conn.cursor() as cur:
                ...
                # --- 💡 NEW: Table for User Spam Data (replaces user_data) ---
                cur.execute("""
                    CREATE TABLE IF NOT EXISTS user_spam_data (
                        user_id BIGINT PRIMARY KEY,
                        blocked_until TIMESTAMP WITH TIME ZONE DEFAULT (NOW() - INTERVAL '1 day'),
                        message_timestamps JSONB DEFAULT '[]'::jsonb
                    );
                """)

                # --- 💡 NEW: Table for Bot Global Data (replaces bot_data) ---
                cur.execute("""
                ...
                # Check/Add columns for chats (if old DB exists)
                try: cur.execute("SELECT chat_type FROM chats LIMIT 1;")
                except psycopg2.ProgrammingError:
                    conn.rollback(); cur.execute("ALTER TABLE chats ADD COLUMN chat_type VARCHAR(50);")
                try: cur.execute("SELECT is_active FROM chats LIMIT 1;")
                except psycopg2.ProgrammingError:
                    conn.rollback(); cur.execute("ALTER TABLE chats ADD COLUMN is_active BOOLEAN DEFAULT TRUE NOT NULL;")
                try: cur.execute("SELECT quiz_message_count FROM chats LIMIT 1;")
                except psycopg2.ProgrammingError:
                    conn.rollback(); cur.execute("ALTER TABLE chats ADD COLUMN quiz_message_count INT DEFAULT 0 NOT NULL;")
...
# ... (register_chat function yahan hai, koi change nahi) ...
...
# ... (update_message_count_db function yahan hai, koi change nahi) ...
async def update_message_count_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    register_chat(update) # Register chat on message
    try:
        with conn.cursor() as cur:
            cur.execute("""
...
# --- Leaderboard Image Generator (Unchanged) ---
# ... (generate_leaderboard_image function yahan hai, koi change nahi) ...
...
# ... (get_leaderboard_data function yahan hai, koi change nahi) ...
...
# ... (format_leaderboard_text function yahan hai, koi change nahi) ...
...
# ... (create_leaderboard_keyboard function yahan hai, koi change nahi) ...
...
# ... (ranking_command function yahan hai, koi change nahi) ...
...
# ... (leaderboard_callback function yahan hai, koi change nahi) ...
...
# ... (profile_command function yahan hai, koi change nahi) ...
...
# ... (send_broadcast_and_handle_errors and broadcast_command function yahan hai, koi change nahi) ...
...
# ... (increment_and_get_quiz_count and reset_quiz_count function yahan hai, koi change nahi) ...
def increment_and_get_quiz_count(chat_id):
    """
    Increments the quiz message counter for a chat in the database
...
def reset_quiz_count(chat_id):
    """
    Resets the quiz message counter for a chat in the database to 0.
    """
    conn = get_db_connection()
    if not conn: return
    try:
...
# ... (get_all_active_chat_ids and deactivate_chat_in_db function yahan hai, koi change nahi) ...
...
# --- 💡 NEW: SPAM PERSISTENCE FUNCTIONS ---
def get_spam_data(user_id):
    """
    Gets spam data (blocked_until, timestamps) for a user from the DB.
    """
    conn = get_db_connection()
    # Return default "not blocked" values on connection error
    default_blocked_until = (datetime.now(timezone.utc) - timedelta(days=1)).timestamp()
    if not conn:
        return (default_blocked_until, [])
    
    try:
        with conn.cursor() as cur:
            # Ensure user exists first
            cur.execute("""
                INSERT INTO user_spam_data (user_id) VALUES (%s)
                ON CONFLICT (user_id) DO NOTHING;
            """, (user_id,))
            
            # Fetch the data
            cur.execute("""
                SELECT blocked_until, message_timestamps
                FROM user_spam_data
                WHERE user_id = %s;
            """, (user_id,))
            row = cur.fetchone()
            ...
            if row:
                blocked_until_ts = row[0].timestamp()
                timestamps_list = row[1] # Already a list from JSONB
                return (blocked_until_ts, timestamps_list)
            
            return (default_blocked_until, [])
    except Exception as e:
        logger.error(f"Failed to get spam data for {user_id}: {e}")
        return (default_blocked_until, [])
    finally:
        if conn: conn.close()

def set_spam_data(user_id, blocked_until_ts, timestamps_list):
    """
    Updates spam data for a user in the DB.
    """
    conn = get_db_connection()
    if not conn: return
    try:
        with conn.cursor() as cur:
            ...
            timestamps_json = json.dumps(timestamps_list)
            
            cur.execute("""
                UPDATE user_spam_data
                SET blocked_until = %s, message_timestamps = %s
                WHERE user_id = %s;
            """, (blocked_until_dt, timestamps_json, user_id))
        conn.commit()
    except Exception as e:
        logger.error(f"Failed to set spam data for {user_id}: {e}")
        conn.rollback()
    finally:
        if conn: conn.close()


# --- 💡 NEW: BOT PERSISTENCE FUNCTIONS ---

def set_bot_value(key, value):
    """
    Sets a value in the bot_persistence table.
    """
    conn = get_db_connection()
    if not conn: return

    # Reset all types
    val_str, val_int, val_float, val_bool, val_json = None, None, None, None, None
    ...
    if isinstance(value, str):
        ...
        logger.error(f"set_bot_value: Unsupported type for key '{key}'")
        return

    try:
        with conn.cursor() as cur:
            cur.execute("""
                INSERT INTO bot_persistence (key, value_str, value_int, value_float, value_bool, value_json)
                VALUES (%s, %s, %s, %s, %s, %s)
            ...
            """, (key, val_str, val_int, val_float, val_bool, val_json))
        conn.commit()
    except Exception as e:
        logger.error(f"Failed to set bot value for key '{key}': {e}")
        conn.rollback()
    finally:
        if conn: conn.close()
...
def get_bot_value(key, default=None):
    """
    Gets a value from the bot_persistence table.
    """
    conn = get_db_connection()
    if not conn: return default
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT value_str, value_int, value_float, value_bool, value_json FROM bot_persistence WHERE key = %s;", (key,))
            row = cur.fetchone()
            if not row:
                return default
            
            # Return the first non-None value
            if row[0] is not None: return row[0]
            if row[1] is not None: return row[1]
            if row[2] is not None: return row[2]
            if row[3] is not None: return row[3]
            if row[4] is not None: return row[4] # Already a dict/list
            
            return default # Should not be reached if row exists
    except Exception as e:
        logger.error(f"Failed to get bot value for key '{key}': {e}")
        return default
    finally:
        if conn: conn.close()
...
def check_and_set_bot_lock(key):
    """
    Atomically checks if a boolean key is False, and if so, sets it to True.
    Returns True if the lock was successfully acquired, False otherwise.
    """
    conn = get_db_connection()
    if not conn: return False # Cannot acquire lock if DB is down
    try:
        with conn.cursor() as cur:
            # Try to get the lock (value_bool = FALSE or row doesn't exist)
            cur.execute("""
                INSERT INTO bot_persistence (key, value_bool)
                VALUES (%s, TRUE)
                ON CONFLICT (key) DO UPDATE
                SET value_bool = TRUE
                WHERE bot_persistence.key = %s AND (bot_persistence.value_bool = FALSE OR bot_persistence.value_bool IS NULL)
                RETURNING key;
            """, (key, key))
            result = cur.fetchone()
            conn.commit()
            ...
            if result:
                ...
                logger.warning(f"Lock NOT acquired, already held: {key}")
                return False # Lock was already TRUE
    except Exception as e:
        logger.error(f"Error checking/setting bot lock for key '{key}': {e}")
        conn.rollback()
        return False
    finally:
        if conn: conn.close()
