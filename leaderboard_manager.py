# leaderboard_manager.py (FINAL VERSION 2: Image Size Adjustment + Caption Name Fix)
...
import re # For stricter name check
...
# --- 🖼️ Leaderboard Image Generator (MODIFIED: Simpler Background Paste/Crop) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str, total_count: int):
    ...
    # --- Load Background Image (Modified Logic) ---
    try:
        ...
        # Create final image with background color fallback
        img = Image.new('RGB', (IMG_WIDTH, total_height), color=COLOR_BG)
        
        # Paste the resized background image, cropping if necessary
        # This keeps the top of the image visible
    ...
    for i, (display_name, count, user_id) in enumerate(leaderboard_data):
        ...
        # 3. Username/User ID
        d.text((160, y_pos), username_display, font=font_text, fill=row_color)
...
# --- Leaderboard Core Logic (Unchanged, fetches user_id) ---
...
# --- Format Leaderboard Text (MODIFIED: Loosened Fallback Logic) ---
def format_leaderboard_text(title: str, chat_name: str, data: list, total_count: int):
    ...
    for rank, (display_name, count, user_id) in enumerate(data, 1):
        ...
        # Loosened check: Only use ID if display name is empty OR contains absolutely NO letters/numbers.
        # This allows names with unrenderable characters (like your 'Baki' example) to still show their name 
        # as long as the readable part (Baki) is present.
...
# --- Profile Command (Modified to use MARKDOWN_V2) ---
