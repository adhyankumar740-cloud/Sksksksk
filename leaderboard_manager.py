# leaderboard_manager.py (FINAL VERSION: Background Image, No Bars, Simple ID Fallback)
...
# --- Message Counting (Unchanged logic) ---
...
# --- 🖼️ Advanced Image Generation (Reverted, only ID prefix removed) ---
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str, total_count: int):
    ...
    for i, (user_id, identifier, count) in enumerate(leaderboard_data):
        ...
        if not display_name or not any(c.isalnum() for c in display_name):
            # 💡 MODIFIED: Removed "User ID: " prefix
...
async def get_leaderboard_data(chat_id: int, scope: str):
    ...
    # Fetches user_id, display_name (username from messages table), and count
    query = f"""
...
# --- 📝 Text Formatting (Reverted, only ID prefix removed) ---
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Using the fixed utility function to get active chats
    ...
    for cid in chat_ids:
        try:
            ...
            # If broadcast fails (bot blocked, chat deleted), deactivate the chat
            deactivate_chat_in_db(cid)
            pass
    
    await update.message.reply_text(f"Done. Success: {count}")

# --- Utility Functions (Fixed in earlier step, kept here to prevent error) ---
...
def deactivate_chat_in_db(chat_id):
    ...
    try:
        ...
        conn.commit()
        logger.info(f"Deactivated chat: {chat_id} due to broadcast failure.")
    except Exception as e:
...
# --- Quiz Counter Utilities (Unchanged) ---
