# leaderboard_manager.py (FINAL VERSION 20: Verified and safe against list index out of range errors)
...
# --- Leaderboard Core Logic (V20 Fixes Retained & Verified) ---
async def get_leaderboard_data(chat_id: int, scope: str, current_user_id: int = None):
    ...
    if current_user_id:
        ...
        try:
            with conn.cursor() as cur:
                ...
                if stats_result:
                    # stats_result[1] is user_rank, stats_result[0] is total_messages
    ...
    try:
        with conn.cursor() as cur:
            ...
            # V19 FIX (Retained in V20): Safely fetch total_count to prevent list index out of range/NoneType error
            ...
            if scope != 'global':
                ...
                # FIX (Retained in V20): Safely fetch chat_name to prevent list index out of range/NoneType error
                if chat_name_result:
        ...
        # This log will only trigger if a deeper error occurs during execution (not an index out of range on fetchone)
...
# --- Utility Functions (V20 Fixes Applied) ---
def get_chat_stats():
    """
    ...
    try:
        with conn.cursor() as cur:
            ...
            result = cur.fetchone()
            # V20 FIX: Safely access [0]
            dm_count = result[0] if result else 0
