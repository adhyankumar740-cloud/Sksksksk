# main.py (Updated with 10-Minute Global Quiz Logic + Auto-Deletion)
...
# 💡 Quiz kitni der khula rahega (seconds)
QUIZ_OPEN_DURATION = 600 
...
# 💡 --- NEW FUNCTION: Job Callback for Deleting Quiz ---
async def delete_quiz_callback(context: ContextTypes.DEFAULT_TYPE):
    """Job callback to delete a poll message after it expires."""
    job = context.job
    chat_id = job.chat_id
    ...
    try:
        ...
        logger.info(f"Successfully deleted expired quiz poll {message_id} from chat {chat_id}.")
    except Exception as e:
        # Agar message pehle hi delete ho gaya ho ya bot admin na ho
        logger.warning(f"Failed to delete poll {message_id} from {chat_id}: {e}")

# 💡 --- MODIFIED FUNCTION: Yeh ab quiz bhejega AUR deletion schedule karega ---
async def send_quiz_and_handle_errors(context: ContextTypes.DEFAULT_TYPE, chat_id, quiz_data):
    try:
        # 1. Quiz bhejenge aur 'sent_message' variable mein store karenge
        sent_message = await context.bot.send_poll(
            ...
            is_anonymous=True,
            open_period=QUIZ_OPEN_DURATION # Poll 10 minute ke liye khula rahega
        )
        
        # 2. Deletion ke liye ek job schedule karenge
        # Hum 2 second extra denge taaki poll server par aaraam se close ho sake
        delete_delay = QUIZ_OPEN_DURATION + 2 
        ...
        context.job_queue.run_once(
            ...
            chat_id=chat_id,
            name=f"del_quiz_{chat_id}_{sent_message.message_id}" # Optional: debugging ke liye
        )
        
        logger.info(f"Quiz {sent_message.message_id} sent to {chat_id}, scheduled for deletion.")
