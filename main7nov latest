# main.py (Updated with Spam Protection)
...
# --- 💡 NEW SPAM CONSTANTS ---
SPAM_MESSAGE_LIMIT = 5 # 5 मैसेज
SPAM_TIME_WINDOW = 2 # 2 सेकंड के अंदर
...
# --- 🎯 CORE MESSAGE COUNTER LOGIC (MODIFIED FOR SPAM PROTECTION) ---
async def send_quiz_after_n_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bot_data = context.bot_data
    user_data = context.user_data # यह उस यूज़र का डेटा है जिसने मैसेज भेजा

    # --- 1. स्पैम प्रोटेक्शन लॉजिक ---
    current_time = time.time()
    
    # जाँचें कि क्या यूज़र ब्लॉक है
    blocked_until = user_data.get('leaderboard_blocked_until', 0)
    is_blocked = False

    if current_time < blocked_until:
        is_blocked = True
        # यूज़र ब्लॉक है, इस मैसेज को न गिनें
    
    else:
        # यूज़र ब्लॉक नहीं है, स्पैम के लिए जाँच करें
        message_timestamps = user_data.get('msg_timestamps', [])
        
        # केवल पिछले X सेकंड की टाइमस्टैम्प रखें
        time_window_start = current_time - SPAM_TIME_WINDOW
        recent_timestamps = [t for t in message_timestamps if t > time_window_start]
        
        # इस मैसेज का टाइमस्टैम्प जोड़ें
        recent_timestamps.append(current_time)
        
        if len(recent_timestamps) >= SPAM_MESSAGE_LIMIT:
            # स्पैम पकड़ा गया!
            is_blocked = True # इस मैसेज को भी न गिनें
            user_data['leaderboard_blocked_until'] = current_time + SPAM_BLOCK_DURATION
            user_data['msg_timestamps'] = [] # टाइमस्टैम्प रीसेट करें
            
            try:
                # यूज़र को चेतावनी दें
                await update.message.reply_text(
                    f"@{update.effective_user.username} <b>You are spamming!</b>\n"
            ...
            # स्पैम नहीं है, टाइमस्टैम्प अपडेट करें
            user_data['msg_timestamps'] = recent_timestamps

    # --- 2. लीडरबोर्ड में मैसेज सेव करें (अगर स्पैम नहीं है) ---
    if not is_blocked:
        # यह लाइन सिर्फ़ तब चलेगी जब यूज़र ब्लॉक नहीं है
        await leaderboard_manager.update_message_count_db(update, context)

    # --- 3. क्विज़ लॉजिक (यह हमेशा की तरह काम करता है) ---
...
def main(): 
    ...
    application.add_handler(
        MessageHandler(
            # MODIFIED: Added filters.User() to make sure we can get user_data
