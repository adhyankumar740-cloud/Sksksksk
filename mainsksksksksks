# main.py (Updated with Persistence + Spam Logic Fix)

import telegram
from telegram import Update, constants
# 💡 FIX 1: PicklePersistence ko import kiya
from telegram.ext import Application, CommandHandler, ContextTypes, MessageHandler, filters, CallbackQueryHandler, PicklePersistence
...
async def send_quiz_after_n_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_id = update.effective_chat.id
    # Ab user_data aur bot_data persistence se aayenge (agar set hai)
    user_data = context.user_data
    bot_data = context.bot_data # Global data

    # --- 1. Spam Protection Logic (Yeh ab persistence ke saath kaam karega) ---
    ...
    if current_time < blocked_until:
        ...
        # Agar block time poora ho gaya hai, toh 'is_blocked' ko explicitly False karein
        # (Warna purana 'leaderboard_blocked_until' value reh sakta hai)
        if 'leaderboard_blocked_until' in user_data:
             # Agar user pehle block tha, toh ek unblock message bhej sakte hain (Optional)
             # logger.info(f"User {update.effective_user.id} is now unblocked.")
             del user_data['leaderboard_blocked_until'] # Purana block time clear karo
        ...
        if len(recent_timestamps) >= SPAM_MESSAGE_LIMIT:
            ...
            logger.info(f"User {update.effective_user.id} blocked for spamming.")
            ...
            user_data['msg_timestamps'] = recent_timestamps # Timestamps update karo

    # --- 💡 FIX 2: LOGIC RESTRUCTURED ---
    
    # --- 2. Update DB (Leaderboard) ---
    # Yeh hissa ab *sirf* tab chalega jab user block NAHI hai
    if not is_blocked:
        # Save Message to Leaderboard
        await leaderboard_manager.update_message_count_db(update, context)
    # Agar user blocked hai, toh upar ka 'if' skip ho jayega aur message count nahi hoga.

    # --- 3. Check for Quiz (Global Logic) ---
    # Yeh hissa *hamesha* chalega, chahe user block ho ya na ho.
    # Isse block hua user bhi quiz trigger kar sakta hai.
...
def main(): 
    ...
    # --- 💡 FIX 1: PERSISTENCE SETUP ---
    # Yeh bot ka data 'bot_data.pkl' naam ki file mein save karega.
    ...
    application = (
        Application.builder()
        .token(TOKEN)
        .persistence(persistence)  # <-- 💡 FIX 1: Persistence ko yahan add kiya
