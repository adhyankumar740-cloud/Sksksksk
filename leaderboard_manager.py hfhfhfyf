...
import os
import psycopg2
from datetime import datetime, timezone, timedelta  # 💡 FIXED: timedelta added and imports set
import logging
import json 
import asyncio
import telegram
...
def generate_leaderboard_image(title: str, leaderboard_data: list, chat_name: str):
    try:
        ...
        font_chat = ImageFont.truetype(FONT_FILE_REGULAR, 30) 
...
def get_spam_data(user_id):
    ...
    # FIX: Use timezone.utc for datetime.now
...
def set_spam_data(user_id, blocked_until_ts, timestamps_list):
    ...
    try:
        with conn.cursor() as cur:
            # Convert timestamp back to datetime object with UTC timezone
    ...
    try:
        with conn.cursor() as cur:
            ...
            if row[4] is not None: 
                 try:
                    return json.loads(row[4]) # Parse JSON string to dict/list
                 except (TypeError, json.JSONDecodeError):
                    logger.error(f"Failed to parse JSON for key '{key}'. Value: {row[4]}")
                    return default
...
def check_and_set_bot_lock(key):
    ...
    try:
        with conn.cursor() as cur:
            # Try to acquire the lock: only set to TRUE if the current value is FALSE or NULL
            cur.execute("""
            ...
            if result:
                ...
                # To prevent spam in logs, only warn if we actively failed to acquire (i.e., row exists)
                cur.execute("SELECT value_bool FROM bot_persistence WHERE key = %s;", (key,))
                current_state = cur.fetchone()
                if current_state and current_state[0] is True:
                     logger.warning(f"Lock NOT acquired, already held: {key}")
                return False # Lock was already TRUE or was null and another race won
    except Exception as e:
