# main.py (Updated: Public Quiz Mode - Shows User Answers)
...
# --- 💡 MODIFIED: Global Broadcast Logic with Message ID Tracking ---
async def broadcast_quiz(context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- 💡 STEP 2: SEND NEW QUIZ AND STORE ID ---
    tasks = [
...
# --- 💡 IMPORTANT MODIFICATION: is_anonymous=False ---
async def send_quiz_and_track_id(context: ContextTypes.DEFAULT_TYPE, chat_id, quiz_data):
    try:
        sent_message = await context.bot.send_poll( 
            ...
            is_anonymous=False, # 💡 MODIFIED: Now Public (Shows who voted what)
            open_period=600 
        )
