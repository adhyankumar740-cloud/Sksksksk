# main.py (Updated with better Spam Constants and Notification)
...
from leaderboard_manager import get_spam_data, set_spam_data, get_bot_value, set_bot_value
...
# --- 💡 SPAM CONSTANTS (Tuned by AI) ---
SPAM_MESSAGE_LIMIT = 8 # 8 messages
SPAM_TIME_WINDOW = 10 # in 10 seconds
SPAM_BLOCK_DURATION = 1200 # 1200 s = 20 minutes (aapke request ke mutabik)
...
# --- 🎯 CORE MESSAGE COUNTER LOGIC (MODIFIED: SPAM BLOCK MESSAGE) ---
async def send_quiz_after_n_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_time < blocked_until:
        ...
        if len(recent_timestamps) >= SPAM_MESSAGE_LIMIT:
            ...
            # 💡 --- Improved Block Notification --- 💡
            block_message = (
                f"<b>You have been blocked for {int(SPAM_BLOCK_DURATION/60)} minutes for spamming.</b>\n\n"
                f"Your messages will not be counted in the leaderboard during this time."
            )
            
            try:
                # 1. Try to send a DM first
                await context.bot.send_message(
                    chat_id=user_id,
                    text=block_message,
                    parse_mode=constants.ParseMode.HTML
                )
                logger.info(f"Sent spam block DM to {user_id}")
            except (telegram.error.Forbidden, telegram.error.BadRequest) as e:
                # 2. If DM fails, send a public reply
                logger.warning(f"Failed to send spam block DM to {user_id}: {e}. Sending public reply.")
                try:
                    username = update.effective_user.username
                    # Use mention_html() for users without usernames
                    mention = f"@{username}" if username else update.effective_user.mention_html()
                    
                    # NOTE: reply_text might fail if the original spam message is deleted
                    # A safer bet is send_message, but reply_text is better for context.
                    await update.message.reply_text(
                        f"{mention}, {block_message.lower()}", # make it lowercase for replies
                        parse_mode=constants.ParseMode.HTML
                    )
                except Exception as e_public:
                    logger.error(f"Failed to send public spam warning for {user_id}: {e_public}")
            # 💡 --- End Improved Notification --- 💡
            
        else:
            # DB में नए timestamps save करो (block time वही रहेगा)
    ...
    # Agar user blocked hai (is_blocked = True), toh yeh function call nahi hoga
    if not is_blocked:
        await leaderboard_manager.update_message_count_db(update, context)
    
    # --- 3. Check for Quiz (Global Logic - Direct Broadcast, No Lock) ---
    
    # DB से last quiz time लाओ
    ...
    if current_time - last_quiz_time > GLOBAL_QUIZ_COOLDOWN:
        ...
        # Timer ko turant reset karo
        leaderboard_manager.set_bot_value(LAST_GLOBAL_QUIZ_KEY, current_time)
        
        logger.info(f"Global quiz cooldown over. Timer reset. Triggered by user {user_id} in chat {chat_id}. Running broadcast directly...")
        
        try:
            # Broadcast ko seedha yahin run karo
            await broadcast_quiz(context)
            logger.info("Direct broadcast finished.")
        except Exception as e:
...
def main(): 
    ...
    logger.info("Starting bot webhook (NO JobQueue, NO Lock)...")
