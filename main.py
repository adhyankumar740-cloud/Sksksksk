...
# --- 💡 NEW SPAM CONSTANTS ---
SPAM_MESSAGE_LIMIT = 10# 5 मैसेज
SPAM_TIME_WINDOW = 4 # 2 सेकंड के अंदर
...
# --- 🎯 CORE MESSAGE COUNTER LOGIC (REORDERED FIX) ---
async def send_quiz_after_n_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- 2. IF NOT BLOCKED: Trigger Quiz AND Update DB (LOGIC REORDERED) ---
    if not is_blocked:
        
        # 2a. Quiz Trigger Logic (MOVED FIRST)
        
        # Check if a quiz is already being sent in this chat
        if not chat_data.get(LOCK_KEY, False): # Only check/increment if not locked
        ...
        # 2b. Save Message to Leaderboard (MOVED SECOND)
        # This runs independently of the quiz logic, but only if user is not blocked.
        try:
            await leaderboard_manager.update_message_count_db(update, context)
        except Exception as e:
            logger.error(f"Failed to update leaderboard DB: {e}")
            # Don't crash the whole handler, quiz logic might have worked
