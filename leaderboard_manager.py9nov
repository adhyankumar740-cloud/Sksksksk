...
# --- Ranking Command (💡 MODIFIED) ---
async def ranking_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    
    # 💡💡💡 FIX 1: DM (Private Chat) Check 💡💡💡
    if update.effective_chat.type == constants.ChatType.PRIVATE:
        await update.message.reply_text("🏆 Leaderboards are only available in group chats.")
        return
    # 💡💡💡 END FIX 1 💡💡💡
...
# --- NEW: SPAM PERSISTENCE FUNCTIONS (Unchanged) ---
...
# --- NEW: BOT PERSISTENCE FUNCTIONS (Unchanged) ---
...
def set_bot_value(key, value):
    ...
    if isinstance(value, str):
        ...
        val_json = json.dumps(value) # Store as JSON string, DB will convert to JSONB
    else:
...
def get_bot_value(key, default=None):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # 💡 FIX: psycopg2 already decodes JSONB columns into dict/list.
            # We don't need to (and shouldn't) call json.loads() on it.
            if row[4] is not None: 
                return row[4] # Just return the value (it's already a dict/list)
...
# (Lock function was already removed, which is correct)
